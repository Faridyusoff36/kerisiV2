...
/**
 * Budget / Budget In Advance (PAGEID 1737 / MENUID 2098) — full list on
 * `budget_in_advance_master`, and read-only header for detail (MENUID 2161).
 */
class BudgetInAdvanceController extends Controller
{
    use ApiResponse;
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $base = $this->list->baseQuery($request, 'in_advance');
        $total = (clone $base)->count();
        $footerTotal = (float) ((clone $base)->sum('bam_total') ?? 0);

        $rows = (clone $base)
            ->tap(fn ($qy) => $this->list->applySort($qy, $sortBy, $sortDir))
            ->skip(($page - 1) * $limit)
            ->take($limit)
            ->get();
    ...
    {
        $row = BudgetInAdvanceMaster::find($id);
        if (! $row) {
            return $this->sendError(404, 'NOT_FOUND', 'Budget in advance record not found');
        }
