...
/**
 * Budget / Budget Advance Controlled (MENUID 2160) or Budget In Advance (MENUID 2098).
 *
 * Legacy: `SNA_API_BUDGET_ADVANCE` on `budget_in_advance_master`. Controlled list uses
 * `bam_total > 0`; “Budget In Advance” uses the full master list. Detail: MENUID 2161.
 */
...
import { listBudgetAdvanceControlled, listBudgetInAdvance } from "@/api/cms";
import { useToast } from "@/composables/useToast";
import type { BudgetAdvanceControlledRow } from "@/types";

const props = withDefaults(
  defineProps<{
    variant?: "controlled" | "in_advance";
  }>(),
  { variant: "controlled" },
);

const PAGE_NAME = computed(() =>
  props.variant === "controlled" ? "List of Budget Advance Controlled" : "Budget In Advance",
);
const PAGE_BREADCRUMB = computed(() =>
  props.variant === "controlled" ? "Budget / Budget Advance Controlled" : "Budget / Budget In Advance",
);

const toast = useToast();
const router = useRouter();

const rows = ref<BudgetAdvanceControlledRow[]>([]);
const loading = ref(false);
const total = ref(0);
const page = ref(1);
const limit = ref(10);
const q = ref("");
const sortBy = ref("tarikh");
const sortDir = ref<"asc" | "desc">("desc");
const footerSum = ref<number | null>(null);
...
function onViewDetail(row: BudgetAdvanceControlledRow) {
  void router.push({ path: "/admin/kerisi/m/2161", query: { id: String(row.bam_id) } });
}

const columns: FimsColumn<BudgetAdvanceControlledRow>[] = [
  { key: "no", label: "No", value: (r) => r.index },
  {
    ...
    sortKey: "bam_year",
    hideable: true,
    value: (r) => (r.bam_year != null ? String(r.bam_year) : ""),
  },
  {
    key: "tarikh",
    label: "Date",
    ...
    sortKey: "tarikh",
    hideable: true,
    value: (r) => formatDateVal(r.tarikh),
  },
  {
    key: "bam_no",
    label: "Reference No",
  ...
  },
  {
    ...
    sortable: true,
    sortKey: "bam_endorse_doc",
    hideable: true,
    value: (r) => r.bam_endorse_doc ?? "",
  },
  {
    key: "bam_total",
    label: "Amount (RM)",
    ...
    hideable: true,
    align: "right",
    value: (r) => (r.bam_total != null ? currency.format(Number(r.bam_total)) : ""),
  },
  {
    key: "bam_status",
    label: "Status",
    ...
    sortKey: "bam_status",
    hideable: true,
    value: (r) => r.bam_status ?? "",
  },
  { key: "action", label: "Action", hideable: true, exportable: false },
];
  ...
  try {
    ...
    const res =
      props.variant === "controlled"
        ? await listBudgetAdvanceControlled(`?${params.toString()}`)
        : await listBudgetInAdvance(`?${params.toString()}`);
    rows.value = res.data;
    total.value = Number(res.meta?.total ?? 0);
    const footer = res.meta?.footer as Record<string, unknown> | undefined;
    const fs = Number(footer?.bam_total_sum);
    footerSum.value = Number.isFinite(fs) ? fs : null;
  } catch (e) {
...
const {
  ...
  pageName: props.variant === "controlled" ? "List of Budget Advance Controlled" : "Budget In Advance",
  apiDataPath: props.variant === "controlled" ? "/budget/advance-controlled" : "/budget/in-advance",
  defaultExportColumns: ["Year", "Date", "Reference No", "Authority Approval", "Amount (RM)", "Status"],
...
async function exportExcel() {
  try {
    ...
    const ws = wb.addWorksheet(PAGE_NAME.value);
    ...
    a.href = url;
    a.download = `${PAGE_NAME.value.replace(/\s+/g, "_")}_${new Date().toISOString().slice(0, 10)}.xlsx`;
    a.click();
    ...
    <div class="space-y-4">
      ...
      <article class="rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        <div class="space-y-4 p-4">
          ...
          <p v-if="footerSum != null" class="text-xs text-slate-600">
            Grand total (filtered)&nbsp;
            <span class="tabular-nums font-medium text-slate-900">{{ currency.format(footerSum) }}</span>
          </p>
          ...
          <FimsListTable
            ...
            :sort-by="sortBy"
            :sort-dir="sortDir"
            :row-key="(r) => String(r.bam_id)"
            :group-by="(r) => `Year ${r.bam_year ?? '(n/a)'}`"
            min-width="1200px"
            @sort="onSort"
          >
            <template #action="{ row }">
