...
/**
 * Account Payable / Credit Note / Credit Note Form (MENUID 3242 / PAGEID 2676).
 *
 * Source: FIMS registry `kerisi-remaining-registry.generated.ts` entry 3242,
 * BL `QLA_API_AP_CN_FORM`, formSections "Credit Note Head".
 *
 * Layout mirrors the legacy two-column form:
 *   Col 1: Credit Note No · Bill No* · Payee Code/Name · Factoring Code/Name ·
 ...
 *   Col 2: Credit Note Approve Date · (blank) · Pay To Type · Factoring Type ·
 ...
 *
 * Route: /admin/kerisi/m/3242
 *   ?cna_id=X           → edit existing CN
...
import {
  getApCreditNoteForm,
  getApCreditNoteFormBillDetails,
  listApCreditNoteFormCurrencies,
  listApCreditNoteFormCustomerTypes,
  saveApCreditNoteForm,
  searchApCreditNoteFormBills,
  submitApCreditNoteForm,
} from "@/api/cms";
import type {
  ApCnFormHead,
  ApCnFormLine,
...
const cnaId = computed(() => {
  const raw = route.query.cna_id ?? route.query.id;
...
const head = ref<ApCnFormHead>({
  cna_id: null,
  cna_crnote_no: null,
  cna_approve_date: null,
  bim_bills_id: null,
  bim_bills_no: null,
  bim_payto_id: null,
  bim_payto_name: null,
  ...
  cna_description: null,
  cna_status_cd: "DRAFT",
});

const debitLines = ref<ApCnFormLine[]>([]);
const creditLines = ref<ApCnFormLine[]>([]);
...
async function runBillSearch(term: string) {
  ...
  try {
    const res = await searchApCreditNoteFormBills(term);
...
async function loadBillDetails(billNo: string) {
  loading.value = true;
  try {
    const res = await getApCreditNoteFormBillDetails(billNo);
    const d = res.data;
    head.value = {
      ...head.value,
    ...
    debitLines.value = (d.debitLines ?? []) as ApCnFormLine[];
    creditLines.value = (d.creditLines ?? []) as ApCnFormLine[];
  } catch (e) {
...
async function loadExistingCn() {
  if (!cnaId.value) return;
  loading.value = true;
  try {
    const res = await getApCreditNoteForm(cnaId.value);
    const d = res.data;
    head.value = {
      cna_id: d.cnaId ?? null,
      cna_crnote_no: d.cnaCrnoteNo ?? null,
      cna_approve_date: d.cnaApproveDate ?? null,
      bim_bills_id: d.bimBillsId ?? null,
      ...
      bim_bill_amt: d.bimBillAmt ?? null,
      bim_current_rate: d.bimCurrentRate ?? null,
      cna_description: d.cnaDescription ?? null,
      cna_status_cd: d.cnaStatusCd ?? "DRAFT",
    };
    billQuery.value = head.value.bim_bills_no ?? "";
    debitLines.value = (d.debitLines ?? []) as ApCnFormLine[];
    creditLines.value = (d.creditLines ?? []) as ApCnFormLine[];
  } catch (e) {
    toast.error("Load failed", e instanceof Error ? e.message : "Unable to load credit note.");
  } finally {
    loading.value = false;
  }
}
...
// ── edit modal for Credit Note Amount ─────────────────────────────────────
const showModal = ref(false);
const editingLine = ref<ApCnFormLine | null>(null);
const editingDtType = ref<"dt" | "cr">("dt");
const modalCnEntAmt = ref<number>(0);
const modalCnAmt = ref<number>(0);

function openEditModal(line: ApCnFormLine, dtType: "dt" | "cr") {
  editingLine.value = line;
  editingDtType.value = dtType;
  modalCnEntAmt.value = Number(line.crdCnEntAmt ?? 0);
  modalCnAmt.value = Number(line.crdCnAmt ?? 0);
  showModal.value = true;
}

function saveModal() {
  if (!editingLine.value) return;
  editingLine.value.crdCnEntAmt = modalCnEntAmt.value;
  editingLine.value.crdCnAmt = modalCnAmt.value;
  // Recalculate balance
  editingLine.value.crdBalEntAmt = (Number(editingLine.value.bidEntAmt ?? 0) - modalCnEntAmt.value);
  editingLine.value.crdBalAmt = (Number(editingLine.value.bidAmt ?? 0) - modalCnAmt.value);
...
const totalDebit = computed(() =>
  debitLines.value.reduce((s, l) => s + Number(l.crdCnAmt ?? 0), 0),
);
const totalCredit = computed(() =>
  creditLines.value.reduce((s, l) => s + Number(l.crdCnAmt ?? 0), 0),
);

// ── canSubmit / canSave ────────────────────────────────────────────────────
const canSubmit = computed(() => {
  if (isReadonly.value) return false;
  const s = (head.value.cna_status_cd ?? "DRAFT").toUpperCase();
  return s === "DRAFT" || s === "ENTRY" || !cnaId.value;
});
...
function validateHead(): boolean {
  ...
  if (!head.value.cna_description?.trim()) {
    toast.error("Missing description", "Credit Note Description is required.");
    return false;
  }
  return true;
}
...
async function handleSave() {
  ...
  saving.value = true;
  try {
    const res = await saveApCreditNoteForm({
      cnaId: head.value.cna_id,
      bimBillsNo: head.value.bim_bills_no,
      cnaDescription: head.value.cna_description,
      debitLines: debitLines.value,
      creditLines: creditLines.value,
    });
    head.value.cna_id = res.data.cnaId ?? null;
    head.value.cna_crnote_no = res.data.cnaCrnoteNo ?? null;
    head.value.cna_status_cd = res.data.cnaStatusCd ?? "DRAFT";
    if (!cnaId.value && res.data.cnaId) {
      await router.replace({ path: "/admin/kerisi/m/3242", query: { cna_id: res.data.cnaId } });
    }
    toast.success("Saved", `Credit note ${head.value.cna_crnote_no ?? res.data.cnaId} saved.`);
  } catch (e) {
...
async function handleSubmit() {
  const ok = await confirm({
    ...
    message: "Submit will save and mark this credit note as submitted.",
    confirmText: "Submit",
  });
  if (!ok) return;
  ...
  if (!cnaId.value && !head.value.cna_id) return;
  const id = (cnaId.value ?? head.value.cna_id) as string;

  submitting.value = true;
  try {
    const res = await submitApCreditNoteForm(id);
    head.value.cna_status_cd = (res.data as { cnaStatusCd?: string }).cnaStatusCd ?? "ENTRY";
    toast.success("Submitted", "Credit note submitted successfully.");
  } catch (e) {
...
function goBack() {
  void router.push("/admin/kerisi/m/3243");
}
...
watch(() => route.query.cna_id, () => {
  if (cnaId.value) void loadExistingCn();
});
</script>
  ...
  <AdminLayout>
    <div class="space-y-4">
      ...
      <div class="flex items-center gap-2">
        ...
        <h1 class="page-title">Account Payable / Credit Note / Credit Note Form</h1>
      </div>
      ...
      <article class="rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        <section class="grid gap-x-8 gap-y-3 p-4 md:grid-cols-2">
          <!-- Row 1 left: Credit Note No -->
          <div class="flex items-center gap-3">
            <label class="w-44 shrink-0 text-xs font-medium text-slate-700">Credit Note No</label>
            <span class="text-slate-400">:</span>
            <input
              :value="head.cna_crnote_no ?? ''"
              type="text"
              readonly
          ...
          <!-- Row 1 right: Credit Note Approve Date -->
          <div class="flex items-center gap-3">
            <label class="w-44 shrink-0 text-xs font-medium text-slate-700">Credit Note Approve Date</label>
            <span class="text-slate-400">:</span>
            <input
              v-model="head.cna_approve_date"
              type="date"
          ...
          <div class="flex items-start gap-3">
            ...
            <div class="relative flex-1">
              ...
              />
              <button
                v-if="billQuery && !isReadonly && !cnaId"
                type="button"
              ...
              <div
                v-if="billOpen && !isReadonly && !cnaId"
          ...
          <!-- Row 12: Credit Note Description (spans full width) -->
          <div class="flex items-start gap-3 md:col-span-2">
            <label class="w-44 shrink-0 pt-1.5 text-xs font-medium text-slate-700">
              Credit Note Description <span class="text-rose-500">*</span>
            </label>
            <span class="pt-1.5 text-slate-400">:</span>
            <textarea
              v-model="head.cna_description"
              :disabled="isReadonly"
              rows="3"
          ...
          <table class="admin-table-kitchen min-w-[1400px]">
            <thead class="admin-table-thead-sticky">
              <tr>
                ...
                <th class="px-2 py-2 text-right text-xs font-semibold">Credit Note Amount<br />(Currency)</th>
                <th class="px-2 py-2 text-right text-xs font-semibold">Credit Note Amount<br />(MYR)</th>
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-2 py-1.5 text-right text-xs tabular-nums">{{ fmtMoney(line.crdCnEntAmt) }}</td>
                <td class="px-2 py-1.5 text-right text-xs tabular-nums">{{ fmtMoney(line.crdCnAmt) }}</td>
                <td class="px-2 py-1.5 text-right text-xs tabular-nums">{{ fmtMoney(line.crdBalEntAmt) }}</td>
                <td class="px-2 py-1.5 text-right text-xs tabular-nums">{{ fmtMoney(line.crdBalAmt) }}</td>
                <td class="px-2 py-1.5 text-xs">
                  <button
                    v-if="!isReadonly"
                    type="button"
                    title="Edit Credit Note Amount"
      ...
      <!-- Action bar -->
      <div v-if="!isReadonly" class="flex items-center justify-center gap-2 py-2">
        <button
          type="button"
        ...
        >
          <div class="w-full max-w-md rounded-lg bg-white shadow-xl">
            <div class="flex items-center justify-between rounded-t-lg bg-indigo-500 px-4 py-3">
              <h3 class="text-sm font-semibold text-white">Credit Note Amount</h3>
            ...
            <div class="grid gap-3 p-4">
              ...
              <div class="grid grid-cols-[140px_10px_1fr] items-center gap-1">
                <label class="text-xs font-medium text-slate-700">
                  Credit Note Amount (Currency) <span class="text-rose-500">*</span>
                </label>
                <span class="text-slate-400">:</span>
                <input
                  v-model.number="modalCnEntAmt"
                  type="number"
                  step="0.01"
                  min="0"
              ...
              <div class="grid grid-cols-[140px_10px_1fr] items-center gap-1">
                <label class="text-xs font-medium text-slate-700">
                  Credit Note Amount <span class="text-rose-500">*</span>
                </label>
                <span class="text-slate-400">:</span>
                <div class="flex items-stretch">
