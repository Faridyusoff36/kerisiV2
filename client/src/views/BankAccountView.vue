...
onMounted(async () => {
  document.addEventListener("click", onClickOutside); await loadOptions(); await loadRows(); });
onUnmounted(() => {
  document.removeEventListener("click", onClickOutside); if (searchDebounce) clearTimeout(searchDebounce); });
</script>
        ...
        <div class="space-y-4 p-4">
          ...
          <div class="overflow-x-auto rounded-lg border border-slate-200">
            <div :class="rows.length > 10 ? 'max-h-[460px] overflow-y-auto' : ''">
              <table class="admin-table-kitchen w-full min-w-[1000px] text-sm">
