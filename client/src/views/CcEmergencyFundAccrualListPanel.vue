...
/**
 * PAGEID 1637 — Accrual worklist (ZR_CREDITCTRL_EMERGENCYFUND_ACCRUAL_API).
 */
import { computed, onMounted, onUnmounted, ref, watch } from "vue";
import { Download, FileDown, FileSpreadsheet, MoreVertical, Search, X } from "lucide-vue-next";
import { listEmergencyFundAccrualListing } from "@/api/cms";
import { useDatatableFeatures } from "@/composables/useDatatableFeatures";
import type { DatatableRefApi } from "@/composables/useDatatableFeatures";
import { useToast } from "@/composables/useToast";
import type { EmergencyFundAccrualRow } from "@/types";

const toast = useToast();
const datatableRef = ref<DatatableRefApi | null>(null);

const rows = ref<EmergencyFundAccrualRow[]>([]);
const total = ref(0);
const page = ref(1);
const limit = ref(10);
const q = ref("");

type SortKey = "emf_emergency_fund_no" | "emf_apply_date" | "emf_taken_amt" | "emf_status";
...
async function loadRows() {
  ...
  try {
    const res = await listEmergencyFundAccrualListing(`?${params.toString()}`);
    rows.value = res.data;
    total.value = Number(res.meta?.total ?? 0);
  } catch (e) {
    toast.error("Load failed", e instanceof Error ? e.message : "Unable to load accrual list.");
  } finally {
    loading.value = false;
  }
}
...
function asExportRow(r: EmergencyFundAccrualRow) {
  return {
    No: r.listIndex,
    emf_id: r.emfId ?? "",
...
const { templateFileInputRef, onTemplateFileChange, handleDownloadPDF, handleDownloadCSV } = useDatatableFeatures({
  pageName: "Credit Control — Emergency Fund Accrual",
  apiDataPath: "/credit-control/emergency-fund-accrual-listing",
...
async function exportExcel() {
  try {
    ...
    a.href = url;
    a.download = `EF_Accrual_${new Date().toISOString().slice(0, 10)}.xlsx`;
    a.click();
  ...
  <div class="space-y-4">
    ...
    <article ref="datatableRef" class="rounded-lg border border-slate-200 bg-white shadow-sm">
      <div class="flex items-center justify-between border-b border-slate-100 px-4 py-3">
        <h2 class="text-base font-semibold text-slate-900">Accrual list</h2>
            ...
            <table class="w-full min-w-[1100px] text-sm">
              ...
              <tbody>
                ...
                <tr v-else-if="rows.length === 0">
                  <td colspan="10" class="px-3 py-6 text-center text-sm text-slate-500">No records.</td>
                </tr>
                <tr
                  v-for="row in rows"
                ...
                >
                  ...
                  <td class="max-w-[200px] px-3 py-2">{{ row.emfCorrespondenceAdd ?? "-" }}</td>
