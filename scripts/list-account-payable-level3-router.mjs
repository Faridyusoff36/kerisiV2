...
const rows = j.filter((r) => String(r.Menu ?? "").toLowerCase().startsWith("account payable"));
const map = new Map();
for (const r of rows) {
...
console.log("unique Account Payable LEVEL3 MENUIDs:", ids.length);

const router = fs.readFileSync("client/src/router/index.ts", "utf8");
const hidden = fs.readFileSync("client/src/router/kerisi-account-payable-hidden-routes.ts", "utf8");
const inRouter = new Set();
for (const m of router.matchAll(/path:\s*["']\/admin\/kerisi\/m\/(\d+)["']/g)) inRouter.add(m[1]);
const inHidden = new Set();
for (const m of hidden.matchAll(/apHidden\(\s*(\d+)/g)) inHidden.add(m[1]);
