...
const map = new Map();
for (const r of j) {
  if (!/^account receivable/i.test(String(r.Menu ?? ""))) continue;
...
console.log("unique Account Receivable (`Menu` prefix) LEVEL3 MENUIDs:", ids.length);
...
for (const m of hidden.matchAll(/arHidden\(\s*(\d+)/g)) inHidden.add(m[1]);
