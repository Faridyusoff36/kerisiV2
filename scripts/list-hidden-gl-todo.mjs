...
function isGeneralLedgerMenu(menu) {
  const s = String(menu ?? "").toLowerCase();
  return (
    s.includes("general ledger") ||
    s.startsWith("gl>") ||
    s.includes(">gl>") ||
    s.includes("/gl/")
  );
...
for (const r of j) {
  if (!isGeneralLedgerMenu(r.Menu)) continue;
...
console.log("General Ledger hidden remaining", todo.length);
