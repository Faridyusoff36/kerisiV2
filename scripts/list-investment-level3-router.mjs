...
for (const r of JSON.parse(fs.readFileSync(level3Path, "utf8"))) {
  ...
  if (!map.has(id)) map.set(id, { title: r.PAGETITLE, PAGEID: r.PAGEID });
}
const ids = [...map.keys()].sort((a, b) => Number(a) - Number(b));
console.log("unique Investment (`Menu` prefix) LEVEL3 MENUIDs:", ids.length);
...
for (const m of hidden.matchAll(/^\s+investmentHidden\(\s*(\d+)/gm)) inHidden.add(m[1]);
