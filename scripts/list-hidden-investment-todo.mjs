...
const extraRoutesTs = "client/src/router/kerisi-investment-hidden-routes.ts";
if (fs.existsSync(extraRoutesTs)) {
  const src = fs.readFileSync(extraRoutesTs, "utf8");
  const fnRe = /investmentHidden\(\s*(\d+)/g;
  while ((m = fnRe.exec(src))) routeIds.add(m[1]);
}

function isInvestmentMenu(menu) {
  const s = String(menu ?? "").toLowerCase();
  if (s.startsWith("investment>")) return true;
  return false;
}
...
for (const f of LEVEL_FILES) {
  ...
  for (const r of j) {
    if (!isInvestmentMenu(r.Menu)) continue;
    rows.push({ ...r, _file: f });
  }
}
...
console.log("Investment hidden (levels 2–5) remaining", todo.length);
