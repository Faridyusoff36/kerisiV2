...
const extraRoutesTs = "client/src/router/kerisi-student-finance-hidden-routes.ts";
if (fs.existsSync(extraRoutesTs)) {
  const src = fs.readFileSync(extraRoutesTs, "utf8");
  const fnRe = /sfHidden\(\s*(\d+)/g;
  while ((m = fnRe.exec(src))) routeIds.add(m[1]);
}

function isStudentFinanceMenu(menu) {
  const s = String(menu ?? "").toLowerCase();
  return s.startsWith("student finance>");
}
...
for (const f of LEVEL_FILES) {
  ...
  for (const r of j) {
    if (!isStudentFinanceMenu(r.Menu)) continue;
    rows.push({ ...r, _file: f });
  }
}
...
console.log("Student Finance hidden (levels 2–5) remaining", todo.length);
