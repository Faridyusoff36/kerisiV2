...
function isPortalMenu(menu) {
  return String(menu ?? "").toLowerCase().startsWith("portal>");
}
...
for (const f of LEVEL_FILES) {
  ...
  for (const r of j) {
    if (!isPortalMenu(r.Menu)) continue;
    rows.push(r);
  }
}
...
console.log("Portal hidden (levels 2–5) remaining", todo.length);
