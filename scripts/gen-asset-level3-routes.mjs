...
const DEFAULT_REL = { path: "/admin/kerisi/m/1548", label: "Open list of asset" };
...
function assetCallEnd(src, from) {
  const open = src.indexOf("assetHidden(", from);
  if (open === -1) return null;
  let i = open + "assetHidden(".length;
  let depth = 1;
  let inStr = false;
  let quote = "";
  ...
  if (src[j] === ",") j++;
  return { open, argsInnerStart: open + "assetHidden(".length, argsInnerEnd: i - 1, end: j };
}

const ASSET_LEGACY_DESC =
...
const exportIdx = hidden.indexOf("export const kerisiAssetHiddenRoutes");
const tail = exportIdx >= 0 ? hidden.slice(exportIdx) : hidden;

const rows = [];
let iter = 0;
while (true) {
  const idx = tail.indexOf("assetHidden(", iter);
  if (idx === -1) break;
  const r = assetCallEnd(tail, idx);
  if (!r) break;
...
if (rows.length !== 2) {
  console.error("Expected 2 Asset LEVEL3 routes, got", rows.length);
  process.exit(1);
}

let out =
  "    // HIDDEN_PAGE_LEVEL3 Asset (`HIDDEN_PAGE_LEVEL3.json` ∩ `kerisi-asset-hidden-routes`; must stay before `...kerisiAssetHiddenRoutes`).\n";
...
for (const row of rows) {
  ...
  out += `      component: AssetLegacyPlaceholderView,\n`;
  out += `      props: {\n`;
  out += `        title: ${JSON.stringify(row.title)},\n`;
  out += `        breadcrumb: ${JSON.stringify(bc)},\n`;
  out += `        description:\n          ${JSON.stringify(ASSET_LEGACY_DESC)},\n`;
...
fs.writeFileSync("scripts/gen-asset-level3-routes-snippet.txt", out);
console.log("Wrote snippet", rows.map((r) => r.menuId).join(","));
