...
function isPayrollMenu(menu) {
  return String(menu ?? "").toLowerCase().startsWith("payroll>");
}
...
for (const f of LEVEL_FILES) {
  ...
  for (const r of j) {
    if (!isPayrollMenu(r.Menu)) continue;
    rows.push(r);
  }
}
...
console.log("Payroll hidden (levels 2–5) remaining", todo.length);
