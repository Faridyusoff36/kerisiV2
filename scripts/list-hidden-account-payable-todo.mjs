...
function isAccountPayableMenu(menu) {
  const s = String(menu ?? "");
  return s.includes("Account Payable") || s.includes("Accounts Payable");
}

const byMenu = new Map();
for (const r of j) {
  if (!isAccountPayableMenu(r.Menu)) continue;
...
console.log("Account Payable hidden remaining", todo.length);
