...
function isProjectMonitoringMenu(menu) {
  return String(menu ?? "").toLowerCase().startsWith("project monitoring>");
}
...
for (const f of LEVEL_FILES) {
  ...
  for (const r of j) {
    if (!isProjectMonitoringMenu(r.Menu)) continue;
    rows.push(r);
  }
}
...
console.log("Project Monitoring hidden (levels 2–5) remaining", todo.length);
