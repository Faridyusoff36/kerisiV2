...
const hiddenPath = "client/src/router/kerisi-general-ledger-extra-hidden-routes.ts";

const j = JSON.parse(fs.readFileSync(level3Path, "utf8"));
const rows = j.filter((r) => /^general ledger/i.test(String(r.Menu ?? "")));
const map = new Map();
for (const r of rows) {
...
console.log("unique General Ledger (`Menu` prefix) LEVEL3 MENUIDs:", ids.length);
...
for (const m of hidden.matchAll(/glExtraHidden\(\s*(\d+)/g)) inHidden.add(m[1]);
